...
const CbseResultFormXII = ({ cbse }) => {
  ...
  const onSubmit = async (data) => {
    ...
    await cbseResultClassXService.createCbseResultClassXII({
      ...data,
      session,
      topper: top,
      achiever: achiev,
  ...
  const onUpdate = async (data) => {
    ...
    await cbseResultClassXService.updateCbseResultClassXII(cbse.$id, {
      ...data,
      ...removeLasts,
    });
    router.push("/admin/dashboard/cbseResult-XII");
    toast.success("Data uploaded successfully", { position: "top-right" });
  };
      ...
      <div className="flex gap-6">
        ...
        {cbse && (
          <Button
            label="Cancel"
            className="mt-4"
            onClick={() => router.push(`/admin/dashboard/cbseResult-XII`)}
          />
        )}
      </div>
    </div>
...
export default CbseResultFormXII;
