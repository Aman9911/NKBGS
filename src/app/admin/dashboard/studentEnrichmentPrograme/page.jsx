...
const StudentEnrichmentProgramePage = () => {
  const [development, setDevelopment] = useState([]);

  const handleFileChange = async (file) => {
    const image = file ? await pdfUploadService.uploadFile(file) : null;
    if (image) {
      const data = await studentEnrichmentService.createStudentEnrichment({
        name: image.name,
        link: image.$id,
  ...
  const handleDelete = (data) => {
    if (data) {
      studentEnrichmentService.deleteStudentEnrichment(data.$id);
...
export default StudentEnrichmentProgramePage;
