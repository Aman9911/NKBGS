...
const CbseResultX = () => {
  ...
  useEffect(() => {
    cbseResultClassXService.getCbseResultClassX().then(async (data) => {
      if (data) {
  ...
  const handleClick = (index) => {
    const updatedNav = result.map((item, idx) => ({
      ...item,
    }));
  ...
  if (loading) {
    return (
      <Container>
        <div className="flex flex-row gap-2 flex-wrap animate-pulse mt-2">
          {Array.from({ length: 6 }).map((_, index) => (
            <button
              key={index}
              className={`relative py-2 px-4 rounded w-20 h-10  bg-gray-300 `}
            >
              <span
        ...
        {Array.from({ length: 1 }).map((_, index) => (
          <div key={index} className="max-w-screen mx-auto p-5 sm:p-10  ">
            <div className="mb-10 rounded overflow-hidden flex flex-col mx-auto">
              <h1 className="text-xl text-center uppercase sm:text-4xl font-semibold text-indigo-600 transition duration-500 ease-in-out inline-block mb-2"></h1>
              <h1 className="text-xl text-center uppercase sm:text-4xl font-semibold text-indigo-600 transition duration-500 ease-in-out inline-block mb-2"></h1>
              <h1 className="text-xl text-center uppercase sm:text-4xl font-semibold text-indigo-600 transition duration-500 ease-in-out inline-block mb-2"></h1>
              <h1 className="text-xl text-center uppercase sm:text-4xl font-semibold text-indigo-600 transition duration-500 ease-in-out inline-block mb-2"></h1>
            </div>
            ...
            <div className="mb-10 overflow-hidden flex flex-col mx-auto">
              ...
              <div className="flex flex-row flex-wrap justify-center ">
                {Array.from({ length: 4 }).map((_, index) => (
                  <StudentCard key={index} tag="loading" />
                ))}
              </div>
            </div>
                      ...
                      {item.subjectWise.map((item, index) => (
                        <React.Fragment key={index}>
                          ...
                          {item?.data?.map((data, index) => (
                            <tr className="border-b" key={data.rollNo}>
